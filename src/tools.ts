            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    max_depth: {
                        type: 'number',
                        description:
                            'Optional maximum depth to recurse. Defaults to unlimited. Must be a positive integer.',
                    },
                },
                required: ['path'],
            },
...
async function runToolEntry(entry: RuntimeToolEntry, rawArguments: unknown): Promise<ToolExecutionOutcome> {
    ...
    const absolutePathError = validateAbsolutePaths(
        rawArguments,
        entry.absolutePathKeys,
        entry.absolutePathErrorMessage
    );
